...
interface ActionType {
    ...
    groupHandler : (fun: onActionType, key?: string) => (...params: any) => Promise<any>;
    keys: string[];
}
...
const useLoading = (onAction?: onActionType): ActionType => {
    ...
    const removeKey = (key?: string) => {
        ...
        setActiveKey(keys => {
            const index = keys.findIndex(k => k === key);
            return index === -1 ? keys : keys.slice(index, 1)
        })
    };
    ...
    const groupHandler = (fun: onActionType, key?: string) => async (...params: any) => {
        callRef.current += 1;
        addKey(key)
        load();
    ...
    return {
        isLoading, isError, handleAction, groupHandler , keys: activeKey,
    };
}
